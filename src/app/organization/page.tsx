...
function SparkNetworkBg() {
  ...
  useEffect(() => {
    ...
    if (!canvas) return;
    const create = function create(e: HTMLCanvasElement, t: () => Cfg) {let a: number,l=e.getContext("2d")!,i: {x:number,y:number}[]=[],n: {a:number,b:number}[]=[],r: {edge:number,t:number,speed:number,dir:number}[]=[],o=-1,s=-1,h=(t: number)=>{let a=e.width,l=e.height;i=Array.from({length:t},()=>({x:.05*a+.9*Math.random()*a,y:.05*l+.9*Math.random()*l})),n=[];for(let e=0;e<t;e++)i.map((t,a)=>({j:a,d:Math.hypot(t.x-i[e].x,t.y-i[e].y)})).filter(t=>t.j!==e).sort((e,t)=>e.d-t.d).slice(0,3).forEach(({j:t})=>{n.some(a=>a.a===e&&a.b===t||a.a===t&&a.b===e)||n.push({a:e,b:t})});o=t},d=(e: number)=>{r=Array.from({length:e},()=>({edge:Math.floor(Math.random()*Math.max(1,n.length)),t:Math.random(),speed:.5+.5*Math.random(),dir:Math.random()>.5?1:-1})),s=e},m=()=>{let a=e.getBoundingClientRect();e.width=a.width,e.height=a.height,h(Math.floor(t().nodeCount)),d(Math.floor(t().sparkCount))},c=()=>{let m=t(),p=e.width,g=e.height,u=Math.floor(m.nodeCount),b=Math.floor(m.sparkCount);u!==o?(h(u),d(b)):b!==s&&n.length>0&&d(b),l.fillStyle=`rgba(11,22,40,${.15+(1-m.trailLength)*.72})`,l.fillRect(0,0,p,g),n.forEach(e=>{l.beginPath(),l.moveTo(i[e.a].x,i[e.a].y),l.lineTo(i[e.b].x,i[e.b].y),l.strokeStyle=`hsla(${m.hue},60%,60%,${m.edgeOpacity})`,l.lineWidth=.5,l.stroke()}),i.forEach(e=>{l.beginPath(),l.arc(e.x,e.y,1.5,0,2*Math.PI),l.fillStyle=`hsla(${m.hue},60%,70%,${2*m.edgeOpacity})`,l.fill()}),r.forEach(e=>{if(e.edge>=n.length)return;e.t+=m.sparkSpeed*e.speed*e.dir*.006,e.t>1&&(e.t=0,e.dir=Math.random()>.5?1:-1),e.t<0&&(e.t=1,e.dir=Math.random()>.5?1:-1);let t=n[e.edge],a=i[t.a].x+(i[t.b].x-i[t.a].x)*e.t,r=i[t.a].y+(i[t.b].y-i[t.a].y)*e.t,o=l.createRadialGradient(a,r,0,a,r,8);o.addColorStop(0,`hsla(${m.hue+40},100%,95%,${m.brightness})`),o.addColorStop(.4,`hsla(${m.hue},90%,70%,${.4*m.brightness})`),o.addColorStop(1,"rgba(0,0,0,0)"),l.beginPath(),l.arc(a,r,8,0,2*Math.PI),l.fillStyle=o,l.fill()}),a=requestAnimationFrame(c)};m(),c();let p=new ResizeObserver(()=>m());return p.observe(e),()=>{cancelAnimationFrame(a),p.disconnect()}};
...
function FlowGlowBg() {
  ...
  useEffect(() => {
    const canvas = canvasRef.current;
    if (!canvas) return;
    const create = function create(e: HTMLCanvasElement, t: () => Cfg) {let a: number,l=e.getContext("2d")!,i=0,n=0,r=performance.now(),o: [number,number,number][]= [[188,130,243],[245,185,234],[141,159,255],[255,103,120],[255,186,113],[198,134,255]],s=[{fx:.31,fy:.47,px:0,py:.5*Math.PI},{fx:.53,fy:.29,px:Math.PI,py:1.2*Math.PI},{fx:.41,fy:.67,px:.7*Math.PI,py:0},{fx:.23,fy:.53,px:1.5*Math.PI,py:.8*Math.PI},{fx:.59,fy:.37,px:.3*Math.PI,py:1.7*Math.PI},{fx:.43,fy:.61,px:1.1*Math.PI,py:.3*Math.PI}],h=["#BC82F3","#F5B9EA","#8D9FFF","#FF6778","#FFBA71","#C686FF"],d=[...h],m=[...h],c=1,p=.45,g=(e: string)=>{let t=parseInt(e.replace("#",""),16);return[t>>16&255,t>>8&255,255&t] as [number,number,number]},u=()=>{let t=e.getBoundingClientRect();e.width=t.width||window.innerWidth,e.height=t.height||window.innerHeight},b=(e: number,t: number,a: number,i: number,n: number)=>{n=Math.max(0,Math.min(n,a/2,i/2)),l.beginPath(),l.moveTo(e+n,t),l.lineTo(e+a-n,t),l.arcTo(e+a,t,e+a,t+n,n),l.lineTo(e+a,t+i-n),l.arcTo(e+a,t+i,e+a-n,t+i,n),l.lineTo(e+n,t+i),l.arcTo(e,t+i,e,t+i-n,n),l.lineTo(e,t+n),l.arcTo(e,t,e+n,t,n),l.closePath()},f=(u: number)=>{let y=Math.min((u-r)/1e3,.05);r=u;let M=t(),x=e.width,w=e.height,k=x/2,S=w/2,v=Math.min(x,w);i+=M.speed*y,n+=.4*y,c=Math.min(1,c+y/.55),(p-=y)<=0&&c>=1&&(p=.4+.15*Math.random(),d=[...m],c=0,m=[...h].sort(()=>Math.random()-.5));let C=d.map((e,t)=>((e: string,t: string,a: number)=>{let[l,i,n]=g(e),[r,o,s]=g(t);return`rgb(${Math.round(l+(r-l)*a)},${Math.round(i+(o-i)*a)},${Math.round(n+(s-n)*a)})`})(e,m[t],c));l.clearRect(0,0,x,w),l.fillStyle="#0B1628",l.fillRect(0,0,x,w);let R=M.padding,F=M.strokeWidth,A=M.intensity,I=x-2*R,P=w-2*R,$=M.cornerRadius,z=v*M.blobSize;l.save(),b(R,R,I,P,$),l.clip(),l.globalCompositeOperation="screen",s.forEach((e,t)=>{let[a,n,rr]=o[t],ss=k+.4*I*Math.sin(i*e.fx+e.px),hh=S+.4*P*Math.cos(i*e.fy+e.py),dd=l.createRadialGradient(ss,hh,0,ss,hh,z);dd.addColorStop(0,`rgba(${a},${n},${rr},${.82*A})`),dd.addColorStop(.42,`rgba(${a},${n},${rr},${.36*A})`),dd.addColorStop(1,`rgba(${a},${n},${rr},0)`),l.beginPath(),l.arc(ss,hh,z,0,2*Math.PI),l.fillStyle=dd,l.fill()}),l.restore();let B=(ee: number,t: number,a: number)=>{let ii: CanvasGradient;l.save(),t>0&&(l.filter=`blur(${t}px)`),l.globalAlpha=a,ii=l.createConicGradient(n-Math.PI/2,k,S),C.forEach((e,t)=>ii.addColorStop(t/C.length,e)),ii.addColorStop(1,C[0]),l.strokeStyle=ii,l.lineWidth=ee,l.lineCap="round",b(R,R,I,P,$),l.stroke(),l.restore()};B(4.5*F,26*A,.3*A),B(2.8*F,14*A,.5*A),B(1.6*F,5*A,.72*A),B(F,0,1),a=requestAnimationFrame(f)};u(),a=requestAnimationFrame(f);let y=new ResizeObserver(()=>u());return y.observe(e),()=>{cancelAnimationFrame(a),y.disconnect()}};
    return create(canvas, () => cfgRef.current);
  }, []);
