...
function CtaCanvas() {
  ...
  useEffect(() => {
    ...
    if (!canvas) return;
    // @ts-ignore
    const create = function create(e,t){let a,l,i=e.getContext("2d"),n,r=0,o=-1,s=e=>{let t=43758.5453*Math.sin(e);return t-Math.floor(t)},h=(e,t)=>{let a=Math.floor(e),l=Math.floor(t),i=e-a,n=t-l,r=i*i*(3-2*i),o=n*n*(3-2*n);return s(a+57*l)*(1-r)*(1-o)+s(a+1+57*l)*r*(1-o)+s(a+(l+1)*57)*(1-r)*o+s(a+1+(l+1)*57)*r*o},d=(e,t,a)=>{let l=0,i=.5,n=1;for(let r=0;r<a;r++)l+=h(e*n+3.7*r,t*n+1.3*r)*i,n*=2,i*=.5;return l},m=()=>{let t=e.getBoundingClientRect();e.width=t.width,e.height=t.height,i.clearRect(0,0,t.width,t.height),o=-1},c=()=>{r+=.003;let s=t(),h=Math.round(s.count);h!==o&&(t=>{a=new Float32Array(t),l=new Float32Array(t);for(let i=0;i<t;i++)a[i]=Math.random()*e.width,l[i]=Math.random()*e.height;o=t})(h);let m=e.width,p=e.height,g=3*s.speed,u=s.noiseScale,b=Math.round(s.octaves);i.fillStyle=`rgba(8,10,14,${(1-s.fadeAlpha).toFixed(3)})`,i.fillRect(0,0,m,p);for(let e=0;e<h;e++){let t=a[e]/m*u,n=l[e]/p*u,o=(d(t,n+r+.002,b)-d(t,n+r-.002,b))/.004*g,h=-(d(t+.002,n+r,b)-d(t-.002,n+r,b))/.004*g,c=a[e],f=l[e];if(a[e]+=o,l[e]+=h,a[e]<0||a[e]>m||l[e]<0||l[e]>p){a[e]=Math.random()*m,l[e]=Math.random()*p;continue}let y=Math.sqrt(o*o+h*h),M=(s.hue+60*y)%360;i.beginPath(),i.moveTo(c,f),i.lineTo(a[e],l[e]),i.strokeStyle=`hsla(${M},75%,65%,0.55)`,i.lineWidth=.7,i.stroke()}n=requestAnimationFrame(c)};m(),c();let p=new ResizeObserver(()=>m());return p.observe(e),()=>{cancelAnimationFrame(n),p.disconnect()}};
