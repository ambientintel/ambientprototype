...
const MODULE_GROUPS = [
  ...
  },
  {
    ...
    accent: '#3DCC91',
    accentDim: 'rgba(61,204,145,0.10)',
    cols: 2,
    modules: [
      { href: '/engineering', tag: 'Engineering', label: 'Engineering Board',   description: 'Sprint tracker, issue board, team roster, and active critical path for the Ambient Intelligence development team. Sprint 18 live.', meta: 'Sprint 18 · 16 issues · Apr–May' },
      { href: '/umn',         tag: 'Systems',     label: 'System Engineering', description: 'Full milestone timeline from clinical need definition through SaMD classification, EVT hardware, cloud architecture, and verification.', meta: 'Concept → Hardware → Cloud → SaMD' },
    ],
  },
  {
    key: 'people',
  ...
  },
  {
    ...
    cols: 3,
    modules: [
      { href: '/bi',          tag: 'Business Intel', label: 'Business Intelligence', description: 'Revenue trends, payer mix, occupancy, fall-prevention ROI, and facility benchmark. Q1 2026 actuals with Q2 forecast.', meta: '$496.8K MRR · 30% margin · Q1 2026' },
  ...
  },
  {
    ...
    modules: [
      ...
      { href: '/contractlab',  tag: 'Legal',    label: 'Contract Lab',      description: 'AI-assisted contract authoring, multi-party e-signature workflows, and document status tracking for vendor and partner agreements.', meta: 'NDA · MSA · SOW · E-sign' },
    ],
  },
  {
    key: 'tools',
...
function MiniLivePanel() {
  const [tick, setTick] = React.useState(0);
  React.useEffect(() => {
    const id = setInterval(() => setTick(t => t + 1), 2200);
    return () => clearInterval(id);
  }, []);
  const event = TICKER_EVENTS[tick % TICKER_EVENTS.length];
  const ROOMS = [
    {id:'301',status:'alert' as RoomStatus},{id:'207',status:'alert' as RoomStatus},
    {id:'104',status:'movement' as RoomStatus},{id:'312',status:'movement' as RoomStatus},
    {id:'205',status:'quiet' as RoomStatus},{id:'118',status:'quiet' as RoomStatus},
    {id:'309',status:'quiet' as RoomStatus},{id:'114',status:'quiet' as RoomStatus},
  ];
  return (
    <div style={{
      ...
      <div style={{ padding:'14px 18px', borderBottom:'1px solid rgba(255,255,255,0.06)' }}>
        <div style={{ display:'flex', alignItems:'center', gap:7, marginBottom:14 }}>
          <span style={{ width:5, height:5, borderRadius:'50%', background:'#3DCC91', boxShadow:'0 0 6px #3DCC91', flexShrink:0 }}/>
          <span style={{ fontFamily:'var(--mono)', fontSize:9, letterSpacing:'0.18em', textTransform:'uppercase', color:'rgba(255,255,255,0.35)' }}>Live · MOH Facility</span>
        </div>
        <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:5 }}>
          {ROOMS.map(r => (
            <div key={r.id} style={{
              padding:'7px 4px', borderRadius:3,
              background:VIZ_STATUS_DIM[r.status],
              border:`1px solid ${VIZ_STATUS_COLOR[r.status]}33`,
              textAlign:'center',
            }}>
              <div style={{ fontFamily:'var(--mono)', fontSize:8, color:VIZ_STATUS_COLOR[r.status], marginBottom:4 }}>{r.id}</div>
              <div style={{ width:4, height:4, borderRadius:'50%', background:VIZ_STATUS_COLOR[r.status], margin:'0 auto', boxShadow: r.status==='alert' ? `0 0 5px ${VIZ_STATUS_COLOR[r.status]}` : 'none' }}/>
            </div>
          ))}
        </div>
      </div>
      <div style={{ padding:'11px 18px' }}>
        <div style={{ display:'flex', alignItems:'center', gap:8 }}>
          <span style={{ width:5, height:5, borderRadius:'50%', background:TICK_COLOR[event.type], flexShrink:0, boxShadow: event.type==='alert' ? `0 0 5px ${TICK_COLOR[event.type]}` : 'none' }}/>
          <span style={{ fontFamily:'var(--mono)', fontSize:9.5, color:'rgba(237,238,240,0.7)', flex:1, lineHeight:1.3 }}>{event.label} · Room {event.room}</span>
          <span style={{ fontFamily:'var(--mono)', fontSize:9, color:'rgba(255,255,255,0.25)', flexShrink:0 }}>{event.ago}</span>
        </div>
      </div>
    </div>
  );
    ...
    <>
      ...
      <div style={{ background: C.bg, minHeight: '100vh', color: C.text, fontFamily: 'var(--sans)' }}>
        ...
        <section style={{ borderTop:`1px solid rgba(255,255,255,0.07)` }}>
          <div style={{ padding:'48px 48px 0', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
            <div>
              <div style={{ fontFamily:'var(--mono)', fontSize:10, letterSpacing:'0.22em', textTransform:'uppercase', color:C.accent, marginBottom:10 }}>Live platform status</div>
              <div style={{ fontFamily:'var(--serif)', fontWeight:300, fontSize:'clamp(22px,2.5vw,32px)', letterSpacing:'-0.02em', color:C.text }}>
                Real-time · sensor to screen.
              </div>
            </div>
            <div style={{ display:'flex', alignItems:'center', gap:8 }}>
              <span style={{ width:6, height:6, borderRadius:'50%', background:'#3DCC91', boxShadow:'0 0 8px #3DCC91' }}/>
              <span style={{ fontFamily:'var(--mono)', fontSize:10, letterSpacing:'0.14em', textTransform:'uppercase', color:C.text3 }}>All systems nominal</span>
            </div>
          </div>
        ...
        <footer style={{ borderTop:`1px solid ${C.border}` }}>
          <div style={{ padding:'32px 48px', display:'grid', gridTemplateColumns:'1fr 1fr 1fr 1fr', gap:0, borderBottom:`1px solid ${C.border}` }}>
            {[
              { label:'Clinical', links:[['Nurse Dashboard','/dashboard/overview'],['Floor Map','/dashboard/floormap'],['Alerts','/dashboard/alerts'],['Analytics','/dashboard/analytics']] },
              { label:'Platform', links:[['Control Center','/control'],['Engineering','/engineering'],['Business Intelligence','/bi'],['Data Science','/datascience']] },
              { label:'Compliance', links:[['SaMD Dashboard','/samd'],['21 CFR 820 Gap','/gapanalysis'],['Bill of Materials','/bom'],['Cloud Infrastructure','/cloud']] },
              { label:'Tools', links:[['System Engineering','/umn'],['Mobile Alert Lab','/mobilelab'],['Contract Lab','/contractlab'],['Brand & Design','/brand']] },
            ].map((col, ci) => (
              <div key={col.label} style={{ paddingRight:32, borderRight: ci < 3 ? `1px solid ${C.border}` : 'none', paddingLeft: ci > 0 ? 32 : 0 }}>
                <div style={{ fontFamily:'var(--mono)', fontSize:9, letterSpacing:'0.2em', textTransform:'uppercase', color:C.text3, marginBottom:16 }}>{col.label}</div>
                <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
                  {col.links.map(([label, href]) => (
                    <Link key={href} href={href} style={{ textDecoration:'none' }}>
                      <span style={{ fontFamily:'var(--mono)', fontSize:11, color:C.text2, letterSpacing:'0.02em', transition:'color 0.15s' }}
                        onMouseEnter={e=>(e.currentTarget.style.color=C.text)}
                        onMouseLeave={e=>(e.currentTarget.style.color=C.text2)}>{label}</span>
                    </Link>
                  ))}
                </div>
              </div>
