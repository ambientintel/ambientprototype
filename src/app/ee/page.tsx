  ...
  useEffect(() => {
    ...
    function init() {
      ...
      pts = Array.from({ length: n }, () => ({
        ...
        r: Math.random() < 0.14 ? 2.6 : Math.random() * 1.1 + 0.7, // ~14% hub nodes
    ...
    function frame() {
      ...
      for (let i = 0; i < pts.length; i++) {
        for (let j = i + 1; j < pts.length; j++) {
          ...
          if (d < maxD) {
            const t = 1 - d / maxD;   // 0→1 as d→0
            // shared activation factor: brighter when both nodes are mid-pulse
      ...
      // Nodes
      pts.forEach(p => {
        const glow = 0.5 + 0.5 * Math.sin(p.pulse); // 0→1 pulsing
...
const LS_KEY       = 'ambient-ee-checklist-v1';
const LS_FREEZE_KEY = 'ambient-ee-frozen-v1';
