...
function LissajousCanvas() {
  ...
  useEffect(() => {
    ...
    let t = 0;
    const figs = [
      { a: 1, b: 2, cx: 0.12, cy: 0.25, r: 130, ph: 0.0 },
      { a: 3, b: 2, cx: 0.48, cy: 0.12, r: 155, ph: 0.4 },
      { a: 5, b: 4, cx: 0.80, cy: 0.28, r: 115, ph: 0.9 },
      { a: 3, b: 4, cx: 0.22, cy: 0.70, r: 105, ph: 1.3 },
      { a: 5, b: 3, cx: 0.62, cy: 0.62, r: 145, ph: 0.7 },
      { a: 2, b: 3, cx: 0.87, cy: 0.80, r: 95,  ph: 0.2 },
      { a: 4, b: 3, cx: 0.42, cy: 0.88, r: 125, ph: 1.8 },
    ];
...
export default function WebAppPage() {
  ...
  const [deployFrozen, setDeployFrozen] = useState(false);
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 16, paddingLeft: 24 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 0 }}>
                    ...
                    <svg width="6" height="10" viewBox="0 0 6 10" fill="none" style={{ flexShrink: 0 }}>
                      <path d="M1 1l4 4-4 4" stroke={ready ? '#2563EB' : '#D1D5DB'} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
                    </svg>
                  </div>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                    <div style={{ fontFamily: 'var(--mono)', fontSize: 9.5, textTransform: 'uppercase', letterSpacing: '0.14em', color: deployFrozen ? '#059669' : ready ? '#2563EB' : '#9CA3AF' }}>
                      {deployFrozen ? 'Milestone' : 'Goal'}
                    </div>
                    <button
                      onClick={() => ready || deployFrozen ? setDeployFrozen(f => !f) : undefined}
                      style={{
                        display: 'flex', alignItems: 'center', gap: 8,
                        padding: '8px 16px', borderRadius: 9, cursor: ready || deployFrozen ? 'pointer' : 'default',
                        border: deployFrozen ? '1.5px solid #059669' : ready ? '1.5px solid #2563EB' : '1.5px dashed #D1D5DB',
                        background: deployFrozen
                          ? 'linear-gradient(135deg, #ECFDF5, #D1FAE5)'
                          : ready
                          ? 'linear-gradient(135deg, #EFF6FF, #DBEAFE)'
                          : '#F9FAFB',
                        transition: 'all 0.2s',
                        boxShadow: deployFrozen ? '0 0 0 3px rgba(5,150,105,0.12)' : ready ? '0 0 0 3px rgba(37,99,235,0.10)' : 'none',
                      }}
                    >
                      {deployFrozen ? (
                        <svg width="14" height="14" viewBox="0 0 16 16" fill="none">
                          <path d="M8 1v14M1 8h14M3.05 3.05l9.9 9.9M12.95 3.05l-9.9 9.9" stroke="#059669" strokeWidth="1.6" strokeLinecap="round"/>
                        ...
                        <svg width="14" height="14" viewBox="0 0 16 16" fill="none">
                          ...
                          <circle cx="8" cy="8" r="2" fill="#2563EB" opacity="0.3"/>
                        </svg>
                      ) : (
                        <svg width="14" height="14" viewBox="0 0 16 16" fill="none">
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 1, textAlign: 'left' }}>
                        <span style={{ fontFamily: 'var(--sans)', fontSize: 12, fontWeight: 600, color: deployFrozen ? '#059669' : ready ? '#1D4ED8' : '#9CA3AF', letterSpacing: '-0.01em', lineHeight: 1.2 }}>
                          {deployFrozen ? 'Live ✓' : 'Pilot Launch'}
                        </span>
                        <span style={{ fontFamily: 'var(--mono)', fontSize: 9.5, color: deployFrozen ? '#059669' : ready ? '#2563EB' : '#9CA3AF' }}>
                          {deployFrozen ? 'ellamemory.com' : ready ? 'Ready — click to confirm' : `${pct}% complete`}
                        </span>
                      </div>
                    </button>
                  </div>
