...
function StatusBadge({ status }: { status: ComplianceStatus }) {
  ...
  return (
    <span style={{
      ...
      background: m.bg, color: m.color, fontFamily: 'var(--mono)',
    }}>{m.label}</span>
  );
}
  ...
  return (
    <button
      ...
      style={{
        ...
        background: value ? 'rgba(45,114,210,0.13)' : 'var(--surface-1)',
...
function SegmentSelect<T extends string>({
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
      <span style={{ fontSize: 11, color: 'var(--text-3)', textTransform: 'uppercase', letterSpacing: '0.08em', fontFamily: 'var(--mono)' }}>{label}</span>
          ...
          return (
            <button key={o.value}
              ...
              style={{
                ...
                background: active ? 'rgba(45,114,210,0.15)' : 'var(--surface-1)',
...
function ProfileSection({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div style={{ marginBottom: 24 }}>
      <div style={{ fontSize: 10, color: 'var(--text-4)', textTransform: 'uppercase', letterSpacing: '0.14em', fontFamily: 'var(--mono)', marginBottom: 10 }}>{title}</div>
  ...
  return (
    <div style={{ maxWidth: 760 }}>
      <div style={{ marginBottom: 22 }}>
        <h2 style={{ margin: 0, fontSize: 16, fontWeight: 500, color: 'var(--text)' }}>Device Profile</h2>
        <p style={{ margin: '4px 0 0', fontSize: 13, color: 'var(--text-3)' }}>
          Configure your device attributes and target markets. The Standards tracker auto-updates to show only applicable standards.
        </p>
      </div>

      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 24 }}>
        <div style={{ padding: '12px 16px', background: 'var(--surface-1)', border: '1px solid var(--line)', borderRadius: 8 }}>
          <div style={{ fontFamily: 'var(--mono)', fontSize: 22, fontWeight: 500, color: 'var(--text)' }}>{applicable.length}</div>
          <div style={{ fontSize: 11, color: 'var(--text-3)', textTransform: 'uppercase', letterSpacing: '0.08em', marginTop: 2 }}>Applicable standards</div>
        </div>
        <div style={{ padding: '12px 16px', background: 'var(--surface-1)', border: '1px solid var(--line)', borderRadius: 8 }}>
          <div style={{ fontFamily: 'var(--mono)', fontSize: 22, fontWeight: 500, color: critCount > 0 ? '#c04040' : 'var(--text-4)' }}>{critCount}</div>
          <div style={{ fontSize: 11, color: 'var(--text-3)', textTransform: 'uppercase', letterSpacing: '0.08em', marginTop: 2 }}>Critical requirements</div>
        </div>
      </div>
      ...
      <div style={{ marginBottom: 24 }}>
        <div style={{ fontSize: 10, color: 'var(--text-4)', textTransform: 'uppercase', letterSpacing: '0.14em', fontFamily: 'var(--mono)', marginBottom: 10 }}>Target Markets</div>
      ...
      <div style={{ padding: '12px 16px', background: 'var(--surface-1)', border: '1px solid var(--line)', borderRadius: 8, fontSize: 12, color: 'var(--text-3)' }}>
        <strong style={{ color: 'var(--text-2)' }}>{applicable.length} standards</strong> apply to this device profile.
  ...
  if (applicable.length === 0) {
    return (
      <div>
        ...
        <div style={{ padding: '40px 20px', textAlign: 'center', border: '1px dashed var(--line)', borderRadius: 8, color: 'var(--text-4)', fontSize: 13 }}>
    ...
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 8, marginBottom: 20 }}>
        {([
          ...
          <div key={label} style={{ padding: '10px 14px', background: 'var(--surface-1)', border: '1px solid var(--line)', borderRadius: 8 }}>
      ...
      {grouped.map(([category, standards]) => {
        ...
        return (
          <div key={category} style={{ marginBottom: 16 }}>
            ...
            >
              <span style={{ fontFamily: 'var(--mono)', fontSize: 11, fontWeight: 600, color: meta.color, textTransform: 'uppercase', letterSpacing: '0.1em' }}>{category}</span>
              <span style={{ fontSize: 11, color: meta.color + 'aa', marginLeft: 2 }}>{standards.length} standard{standards.length !== 1 ? 's' : ''}</span>
              <div style={{ flex: 1 }} />
              <span style={{ fontFamily: 'var(--mono)', fontSize: 11, color: meta.color }}>
                {catComplete}/{standards.length} complete
              </span>
              <span style={{ color: meta.color, fontSize: 14 }}>{collapsed ? '▶' : '▼'}</span>
            </button>
      ...
      {filtered.length === 0 && (
        <div style={{ padding: '24px', textAlign: 'center', border: '1px dashed var(--line)', borderRadius: 8, color: 'var(--text-4)', fontSize: 13 }}>
...
function StandardRow({
  ...
  // Seed checklist if empty (first open)
  ...
  return (
    <div style={{
      ...
      >
        <span style={{ width: 6, height: 6, borderRadius: '50%', background: priorityColor, flexShrink: 0, marginTop: 1 }} />
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexShrink: 0 }}>
          {prog.total > 0 && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
              <div style={{ width: 48, height: 3, borderRadius: 2, background: 'var(--surface-3)' }}>
                <div style={{ width: `${progressPct}%`, height: '100%', borderRadius: 2, background: progressPct === 100 ? '#3DCC91' : '#2D72D2', transition: 'width 0.2s' }} />
              </div>
          ...
          <span style={{ color: 'var(--text-4)', fontSize: 13 }}>{isOpen ? '▲' : '▼'}</span>
        </div>
      </div>
      ...
      {isOpen && (
        <div style={{ padding: '0 14px 14px', borderTop: '1px solid var(--line)' }}>
          ...
          <div style={{ marginBottom: 14 }}>
            <div style={{ fontSize: 11, color: 'var(--text-3)', textTransform: 'uppercase', letterSpacing: '0.08em', fontFamily: 'var(--mono)', marginBottom: 8 }}>Checklist</div>
            {checklist.map(item => (
              <label key={item.id}
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 10 }}>
            <div>
              <label style={{ fontSize: 11, color: 'var(--text-3)', textTransform: 'uppercase', letterSpacing: '0.08em', fontFamily: 'var(--mono)', display: 'block', marginBottom: 4 }}>Assignee</label>
              <input value={compliance.assignee} onChange={e => onFieldChange('assignee', e.target.value)}
                placeholder="Owner / responsible party"
                style={{ width: '100%', height: 32, background: 'var(--surface-2)', border: '1px solid var(--line)', borderRadius: 6, padding: '0 10px', color: 'var(--text)', fontSize: 12, fontFamily: 'var(--sans)', outline: 'none' }} />
            </div>
            <div>
              <label style={{ fontSize: 11, color: 'var(--text-3)', textTransform: 'uppercase', letterSpacing: '0.08em', fontFamily: 'var(--mono)', display: 'block', marginBottom: 4 }}>Target Date</label>
              <input type="date" value={compliance.targetDate} onChange={e => onFieldChange('targetDate', e.target.value)}
                style={{ width: '100%', height: 32, background: 'var(--surface-2)', border: '1px solid var(--line)', borderRadius: 6, padding: '0 10px', color: 'var(--text)', fontSize: 12, fontFamily: 'var(--sans)', outline: 'none', colorScheme: 'dark' }} />
            </div>
          </div>

          <div>
            <label style={{ fontSize: 11, color: 'var(--text-3)', textTransform: 'uppercase', letterSpacing: '0.08em', fontFamily: 'var(--mono)', display: 'block', marginBottom: 4 }}>Notes</label>
            <textarea value={compliance.notes} onChange={e => onFieldChange('notes', e.target.value)}
              ...
              rows={3}
              style={{ width: '100%', background: 'var(--surface-2)', border: '1px solid var(--line)', borderRadius: 6, padding: '7px 10px', color: 'var(--text)', fontSize: 12, fontFamily: 'var(--sans)', outline: 'none', resize: 'vertical' }} />
          </div>
        </div>
      )}
    </div>
...
// ── Shared helpers (duplicated from page.tsx to avoid coupling) ────────────────
