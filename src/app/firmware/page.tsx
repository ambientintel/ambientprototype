...
// Six sites of nested regular and star polygons — each rotating independently.
// Triangle, hexagon, pentagon, square, octagon, heptagon families.
...
function GeoCanvas() {
  ...
  useEffect(() => {
    ...
    // {n/step}: regular polygon when step=1, star polygon when step>1.
    // polys sorted largest→smallest radius so frac ramps from faint→defined.
    type P = { r:number; n:number; step:number; rs:number; ph:number };
    type Site = { fx:number; fy:number; dr:number; ds:number; dp:number; polys:P[] };

    const SITES: Site[] = [
      // Equilateral triangles — top-left
      { fx:0.14, fy:0.24, dr:0.018, ds:0.000305, dp:0.0, polys:[
        { r:195, n:3, step:1, rs: 0.000110, ph:0.0        },
        { r:145, n:3, step:1, rs:-0.000155, ph:Math.PI/3  },
        { r: 95, n:3, step:1, rs: 0.000210, ph:0.5        },
        { r: 52, n:3, step:1, rs:-0.000290, ph:Math.PI/3+0.5 },
      ]},
      // Hexagons + hexagram — top-right
      { fx:0.78, fy:0.14, dr:0.016, ds:0.000388, dp:1.1, polys:[
        { r:200, n:6, step:1, rs: 0.000088, ph:0.0        },
        { r:200, n:6, step:2, rs: 0.000088, ph:0.0        }, // hexagram at same radius
        { r:138, n:6, step:1, rs:-0.000122, ph:0.3        },
        { r: 88, n:6, step:1, rs: 0.000175, ph:0.6        },
        { r: 48, n:6, step:1, rs:-0.000242, ph:0.9        },
      ]},
      // Pentagons + pentagrams — bottom-center
      { fx:0.52, fy:0.78, dr:0.015, ds:0.000255, dp:2.3, polys:[
        { r:215, n:5, step:1, rs: 0.000075, ph:0.0        },
        { r:215, n:5, step:2, rs: 0.000075, ph:0.0        }, // pentagram overlay
        { r:148, n:5, step:1, rs:-0.000105, ph:0.4        },
        { r:148, n:5, step:2, rs:-0.000105, ph:0.4        },
        { r: 88, n:5, step:1, rs: 0.000148, ph:0.8        },
        { r: 48, n:5, step:1, rs:-0.000210, ph:1.2        },
      ]},
      // Squares, 45°-offset pairs — right
      { fx:0.90, fy:0.60, dr:0.020, ds:0.000335, dp:0.7, polys:[
        { r:170, n:4, step:1, rs: 0.000092, ph:0.0        },
        { r:170, n:4, step:1, rs: 0.000092, ph:Math.PI/4  }, // 45° twin
        { r:115, n:4, step:1, rs:-0.000132, ph:0.2        },
        { r:115, n:4, step:1, rs:-0.000132, ph:Math.PI/4+0.2 },
        { r: 65, n:4, step:1, rs: 0.000188, ph:0.4        },
      ]},
      // Octagons + octagrams — bottom-left
      { fx:0.07, fy:0.82, dr:0.017, ds:0.000278, dp:3.6, polys:[
        { r:190, n:8, step:1, rs: 0.000065, ph:0.0        },
        { r:190, n:8, step:3, rs: 0.000065, ph:0.0        }, // {8/3} octagram
        { r:128, n:8, step:1, rs:-0.000092, ph:0.2        },
        { r:128, n:8, step:3, rs:-0.000092, ph:0.2        },
        { r: 72, n:8, step:1, rs: 0.000138, ph:0.4        },
      ]},
      // Heptagons + heptagram — center
      { fx:0.44, fy:0.43, dr:0.022, ds:0.000448, dp:5.1, polys:[
        { r:175, n:7, step:1, rs: 0.000082, ph:0.0        },
        { r:175, n:7, step:2, rs:-0.000082, ph:0.0        }, // {7/2}
        { r:115, n:7, step:1, rs:-0.000118, ph:0.45       },
        { r:115, n:7, step:3, rs: 0.000118, ph:0.45       }, // {7/3}
        { r: 62, n:7, step:1, rs: 0.000165, ph:0.9        },
      ]},
    ];

    function gcd(a: number, b: number): number { return b === 0 ? a : gcd(b, a % b); }

    function drawPoly(cx: number, cy: number, r: number, n: number, step: number, angle: number) {
      const g = gcd(n, step);
      const segs = n / g;
      for (let c = 0; c < g; c++) {
        ctx.beginPath();
        for (let i = 0; i <= segs; i++) {
          const idx = (c + i * step) % n;
          const a = angle + (idx / n) * Math.PI * 2 - Math.PI / 2;
    ...
    function frame() {
      ...
      // Fine mathematical grid
      ctx.lineWidth = 0.5;
      ctx.strokeStyle = 'rgba(8,14,50,0.026)';
      for (let x = 0; x < W; x += 80) { ctx.beginPath(); ctx.moveTo(x,0); ctx.lineTo(x,H); ctx.stroke(); }
      for (let y = 0; y < H; y += 80) { ctx.beginPath(); ctx.moveTo(0,y); ctx.lineTo(W,y); ctx.stroke(); }

      SITES.forEach(site => {
        const cx = site.fx * W + site.dr * W       * Math.sin(t * site.ds + site.dp);
        const cy = site.fy * H + site.dr * H * 0.6 * Math.cos(t * site.ds * 0.77 + site.dp + 0.5);
        const np = site.polys.length;

        site.polys.forEach((p, i) => {
          const frac  = i / (np - 1 || 1);  // 0=outermost faint → 1=innermost defined
          const alpha = 0.034 + frac * 0.068;
          const lw    = 0.52 + frac * 0.58;
          const angle = p.ph + t * p.rs;

          ctx.strokeStyle = `rgba(8,14,58,${alpha.toFixed(4)})`;
          ctx.lineWidth = lw;
          drawPoly(cx, cy, p.r, p.n, p.step, angle);
        });
      });
